pipeline {
  ...
  post {
    ...
    failure {
      emailext(attachLog: true, body: "Failed build in project  with number build  Failed stage name on ${FAILED_STAGE}", to: "${EMAIL_TO}", subject: 'Failed: $PROJECT_NAME - #$BUILD_NUMBER')
    }
